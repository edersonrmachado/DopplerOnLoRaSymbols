...
STOP_IN_THE_FIRST_SYMBOL = True         # stop or not when have an error
...
if PRINT_TO_FILE or GENERATE_CSV:
    ...
    archive_csv2 = image_folder +'model_droppler_values_complete.csv'
...
def create_symbol_nyquist(SF, s):
    N = 2 ** SF
    nlim = N - s
    symbol = []
    for n in range(N):
        if 0 <= n < nlim:
            sample = np.exp(2*np.pi*1j*(((n**2)/(2*N))+(s/N-0.5)*n))
        elif nlim <= n < N:
            sample = np.exp(2*np.pi*1j*(((n**2)/(2*N))+(s/N-1.5)*n))
        symbol.append(sample)
    return np.array(symbol)

def create_symbol_nyquist_doppler(SF, s, f, f0, fs):
    N = 2 ** SF
    nlim = N - s
    symbol = []
    for n in range(N):
        if 0 <= n < nlim:
            sample = np.exp(2*np.pi*1j*(((n**2)/(2*N))+(s/N-0.5+(f[n]-f0)/fs)*n))
        elif nlim <= n < N:
            sample = np.exp(2*np.pi*1j*(((n**2)/(2*N))+(s/N-1.5+(f[n]-f0)/fs)*n))
        symbol.append(sample)
    return np.array(symbol)

def create_symbol_fs(SF, s, B, fs): 
    N = 2 ** SF
    Ts = 1 / fs
    T = (2 ** SF) / B
    ...
    for n in intervalN:
        if 0 <= n < nlim:
            sample = np.exp(2*np.pi*1j*(((B *Ts*n)**2)/(2*N)+(s/N-1/2)*n*Ts*B)) 
        elif nlim <= n < nAmostras:
            sample = np.exp(2*np.pi*1j*(((B*Ts*n)**2)/(2*N)+(s/N-3/2)*n*Ts*B))
        symbol.append(sample) 
    return np.array(symbol)

def create_symbol_fs_doppler(SF, s, B, fs, f, f0): 
    N = 2 ** SF
    Ts = 1 / fs
    T = (2 ** SF) / B
    nAmostras = int(N * fs / B)
    ...
    for n in intervalN:
        if 0 <= n < nlim:
            sample = np.exp(2*np.pi*1j*(((B*Ts*n)**2)/(2*N)+(s/N-0.5)*n*Ts*B+(f[n]-f0)*n/fs)) 
        elif nlim <= n < nAmostras:
            sample = np.exp(2*np.pi*1j*(((B*Ts*n)**2)/(2*N)+(s/N-1.5)*n*Ts*B+(f[n]-f0)*n/fs))
        symbol.append(sample) 
    return np.array(symbol)

def calculate_estimate_doppler(SF, s, p, Ts, ti, H, f0, finicial):
    N = 2 ** SF
    n = np.arange(0, N)
    fs = 1 / Ts
    t_samples = np.linspace(ti + (p - 1) * N * Ts, ti + p * N * Ts - Ts, N)
    deltaPPM = calculate_deltaF(H, t_samples)
    f = f0 * (deltaPPM + 1)
    for i in range(len(f)):
        values_doppler.append(f[i] - f0)
        sample_values_sf.append(SF)
    
    f = f - finicial
    if fs == B:
        symbol_doppler = create_symbol_nyquist_doppler(SF, s, f, f0, fs)
    else:
        symbol_doppler = create_symbol_fs_doppler(SF, s, B, fs, f, f0)
    # Dechirping
    if fs == B:
        x0 = create_symbol_nyquist(SF, 0)
    else:
        x0 = create_symbol_fs(SF, 0, B, fs)
    x0conj = np.conj(x0)
    yx = symbol_doppler * x0conj
    yx = yx.astype(np.complex64)
    fft_abs = np.abs(np.array(fft(yx)))   
...
def calculate_doppler_graph(SF, B, Ts, line, column, graph,H,f0):
    ...
    ti = 0                      # initial time
    number_symbols = 107        # maximum number of symbols to be estimated
    ...
    finicial = calculateF(H, ti, f0) - f0  # calculate the corrected frequency at synchronization (offset)


    for p in range(1, number_symbols + 1):
        fft_abs, estimate_symbol, peak_value_fft = calculate_estimate_doppler(SF, s, p, Ts, ti, H, f0, finicial)
        ...
        if  estimate_symbol != s and estimate_symbol != previous_estimated_symbol:
            ...
            #print(f'p:{p}', peak_value_fft)   
            print(f'p:{p}') #print p-symbol value of the error 
            if STOP_IN_THE_FIRST_SYMBOL:
                break
            else:
                p_wrong.append(p)   
